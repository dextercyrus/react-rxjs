    ...
    if (!localStorage["BingDefaultsSet"]) {
        ...
        chrome.cookies.getAll({}, function (cookies) {
            ...
            for (var i in cookies) {
                cookieFound = false;
                if (cookies[i].name == "PCCodeSE") {
                    PCCode = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "MachineIDSE") {
                    MachineID = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "OSSE") {
                    OS = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "InstalledDateSE") {
                    InstalledDate = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "InstalledTimeSE") {
                    InstalledTime = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "MarketSE") {
                    Market = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "LoggerVersionSE") {
                    LoggerVersion = cookies[i].value;
                    cookieFound = true;
                }
                else if (cookies[i].name == "PackageCodeSE") {
...
function SendPing(PCCode,MachineID,OS,InstalledDate,InstalledTime, LoggerVersion, Market, PackageCode)
{
	...
	var TVData = 'TV=is'+ PCCode +'|pk'+ PackageCode +'|rt1|tm'+ Market +'|hd' + chromeVersion +',0|sd' + chromeVersion +',1|se' + chromeVersion +',1|hp|cd'+ InstalledDate +'|ct'+ InstalledTime;
